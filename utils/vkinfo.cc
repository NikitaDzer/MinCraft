...
std::string versionToString(uint32_t version) {
  ...
  return fmt::format("{}.{}.{}", ver_major, ver_minor,
...
void printPhysicalDeviceProperties(vk::PhysicalDevice device) {
  ...
  fmt::print(
      ...
      properties.deviceID, vk::to_string(properties.deviceType),
  ...
  const auto extensions_string = std::accumulate(
      ...
      [](auto str, auto ext) { return str + " " + ext.extensionName.data(); } //
  );

  fmt::print("Physical device [{}] supports following extensions: {}\n",
             properties.deviceName,
             extensions_string //
  );
  ...
  for (uint32_t i = 0; const auto &info : qf_info) {
    fmt::print("[{}]. [queue_count = {}] of the type {}\n", i, info.queueCount,
...
int main() try {
  ...
  auto instance = GenericInstance::make<DebuggedInstance>(
      vkwrap::VulkanVersion::e_version_1_3, nullptr,
