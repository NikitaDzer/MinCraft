...
constexpr auto k_application_info = vk::ApplicationInfo{
    .pApplicationName = "vkinfo", .applicationVersion = VK_MAKE_VERSION( 0, 0, 1 ),
    .pEngineName = "no engine", .engineVersion = VK_MAKE_VERSION( 0, 0, 1 ), .apiVersion = VK_VERSION_1_1
};
...
printPhysicalDeviceProperties( vk::PhysicalDevice device )
{
    ...
    fmt::println( "Found physical device: [id = {}, type = {}, name = {}, version = {}]",
                  properties.deviceID, vk::to_string( properties.deviceType ), properties.deviceName, versionToString(properties.apiVersion) );

    const auto extensions_string = std::accumulate( extensions.begin(), extensions.end(), std::string{},
                                                    [] (auto str, auto ext) {
