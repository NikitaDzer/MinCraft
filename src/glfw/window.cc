...
std::string getHandleAddressString( const Window* window )
{
...
void logGLFWaction( std::string &&action, 
...
WindowType* Window::createWindow( const WindowConfig &config,
                                  Window* bound_handle )
{
    setWindowHints();
    ...
    logGLFWaction( "create window",
                   getHandleAddressString( bound_handle ) );

    return glfwWindow;
} // createWindow

void Window::destroyWindow( WindowType* glfwWindow )
{
    Window *window = getHandle( glfwWindow );

    glfwDestroyWindow( glfwWindow );

    logGLFWaction( "destroy window",
                   getHandleAddressString( window ) );
} // destroyWindow

void Window::framebufferSizeCallback( WindowType* glfwWindow, int width, int height )
{
    Window *window = getHandle( glfwWindow );

    logGLFWaction( "framebuffer resize",
                   getHandleAddressString( window ),
...
FramebufferSizeType Window::getFramebufferSize() const&
{
    int width  = 0;
    int height = 0;
...
void Window::setWindowed() const&
{
    ...
    glfwSetWindowMonitor( m_handle.get(), NULL, xpos, ypos, getFramebufferWidth(), getFramebufferHeight(), GLFW_DONT_CARE );

    logGLFWaction( "choose windowed mode",
...
void Window::setFullscreen() const&
{
    ...
    glfwSetWindowMonitor( m_handle.get(), monitor, 0, 0, getFramebufferWidth(), getFramebufferHeight(), GLFW_DONT_CARE );

    logGLFWaction( "choose fullscreen mode",
                   getHandleAddressString( this ) );   
} // setFullscreen
...
bool WindowManager::isSuitableVersion()
{
    int major = 0;
    int minor = 0;
...
void WindowManager::initialize( ErrorCallbackSignature *error_callback )
{
...
std::span<std::string_view> WindowManager::getRequiredExtensions( )
{
        uint32_t size = 0;
